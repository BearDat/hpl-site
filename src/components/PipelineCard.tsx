...
type PipelineEntry = {
  ...
  color: string;
  movement:
    | { direction: "up"; value?: number; isNew?: boolean }
    | { direction: "down"; value?: number }
    | { direction: "flat" };
};

function Movement({ movement }: { movement: PipelineEntry["movement"] }) {
  ...
  if (movement.direction === "up" && movement.isNew) {
    return (
      <div className="w-7 flex-shrink-0 text-right text-[9px] font-extrabold tracking-wide text-[#5fbf8a]">
        NEW
      </div>
    );
  }
...
export function PipelineCard({ entries }: { entries: PipelineEntry[] }) {
  return (
      ...
      {entries.map((entry, i) => (
        <div
          ...
          className={`flex items-center gap-3.5 py-2.5 ${
            i < entries.length - 1 ? "border-b border-paper/15" : ""
          }`}
        >
