...
import { bulkImportGames, type BulkImportState } from "@/lib/actions/schedule";
import { buttonClass } from "./ui";

export function BulkImportForm({ seasonId }: { seasonId: string }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3">
      <textarea
        name="scheduleText"
        rows={8}
        placeholder={"R1\t7PM EST\n\tSt Louis Archers\tNashville Blues  LS1\n\tBoston Bengals\tChicago Breeze  LS2"}
        className="w-full border border-ink/30 bg-white p-3 font-mono text-xs focus:border-accent focus:outline-none"
      />
      ...
      {state && (
        <div className="border border-ink/20 bg-paper p-3 text-sm">
          <p className="font-bold">{state.createdCount} game(s) created.</p>
          {state.errors.length > 0 && (
            <ul className="mt-2 list-disc pl-5 text-[#c1391f]">
