import { addPlayoffGame, updatePlayoffGame, deletePlayoffGame, setSeriesWinner, deleteSeries, } from "@/lib/actions/playoffs";
import { inputClass, buttonClass, buttonSecondaryClass } from "./ui";
export function PlayoffSeriesCard({ series }) {
    const addGameWithId = addPlayoffGame.bind(null, series.id);
    const setWinnerWithId = setSeriesWinner.bind(null, series.id);
    return (<div className="mb-4 border-[3px] border-ink p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          {series.winner && (<span className="font-extrabold text-accent">
      ...
      {series.games.map((game) => {
            ...
            return (<div key={game.id} className="mb-1.5 flex items-center gap-2 border-b border-ink/10 pb-1.5 text-sm last:border-b-0">
      ...
      {series.teamA && series.teamB && (<form action={addGameWithId} className="mt-2 flex flex-wrap items-end gap-2">
      ...
      {series.teamA && series.teamB && (<form action={setWinnerWithId} className="mt-2 flex items-center gap-2">
