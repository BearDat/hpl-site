...
export function PlayerRow({ player, teams }) {
    ...
    return (<form action={updateWithId} className="grid grid-cols-2 items-center gap-2 border-b border-ink/10 py-2 text-sm last:border-b-0 sm:grid-cols-12">
      <Link href={`/players/${player.slug}`} target="_blank" className="col-span-2 truncate font-bold hover:text-accent sm:col-span-3">
        {player.name}
      </Link>
      <input name="name" defaultValue={player.name} className={`${inputClass} col-span-2 sm:col-span-3`}/>
