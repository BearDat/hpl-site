...
export function StatLeadersCard({ leaders }) {
    return (<div className="flex flex-col border-[3px] border-ink p-[18px]">
      <div className="mb-2.5 text-[11px] font-extrabold tracking-wider opacity-55">
        STAT LEADERS
      </div>
      {leaders.map((stat, i) => (<div key={stat.label} className={`flex items-center gap-2.5 py-1.5 ${i < leaders.length - 1 ? "border-b border-ink/10" : ""}`}>
