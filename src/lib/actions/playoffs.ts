...
function roundNameForSeriesCount(n: number): string | undefined {
  if (n === 1) return "Finals";
  if (n === 2) return "Semifinals";
  if (n === 4) return "Quarterfinals";
  return undefined;
}

async function recomputeSeries(seriesId: string) {
  ...
  for (const g of series.games) {
    ...
    if (winnerTeamId && winnerTeamId === series.teamAId) aWins += 1;
    else if (winnerTeamId && winnerTeamId === series.teamBId) bWins += 1;
  }

  const majority = Math.ceil(series.bestOf / 2);
  const winnerId =
    aWins >= majority ? series.teamAId : bWins >= majority ? series.teamBId : null;
...
export async function generateFirstRound(formData: FormData) {
  ...
  if (existing > 0) throw new Error("Round 1 series already exist for this season.");
  ...
  if (seeded.length < 2) throw new Error("Not enough teams to generate a bracket.");
...
export async function generateNextRound(formData: FormData) {
  ...
  if (!seasonId || !fromRound) throw new Error("Season and round are required.");
  ...
  if (nextRoundExists > 0) throw new Error("The next round already exists.");
...
export async function createSeriesManually(formData: FormData) {
  const seasonId = String(formData.get("seasonId") ?? "");
  ...
  if (!seasonId || !round) throw new Error("Season and round are required.");
...
export async function setSeriesWinner(seriesId: string, formData: FormData) {
...
export async function addPlayoffGame(seriesId: string, formData: FormData) {
...
export async function updatePlayoffGame(gameId: string, formData: FormData) {
...
export async function deletePlayoffGame(formData: FormData) {
