...
async function logoUrlFromForm(formData: FormData): Promise<string | undefined> {
...
export async function deleteTeam(formData: FormData) {
  ...
  if (gameCount > 0 || transactionCount > 0) {
    throw new Error(
      "This team has games or transaction history and can't be deleted. Remove those first, or keep the team and rebrand it instead."
    );
  }
