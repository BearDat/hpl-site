    ...
    <div>
      ...
      <Panel title="Seasons">
        <div className="mb-4 flex flex-col gap-2">
          {seasons.map((s) => (
            <div key={s.id} className="flex items-center justify-between border-b border-ink/10 py-2 text-sm">
      ...
      {!current ? (
        <Panel title="Divisions, Teams, Playoffs">
        ...
        <>
          <Panel title={`Divisions — ${current.name}`}>
            <div className="mb-4 flex flex-col gap-2">
              {divisions.map((d) => (
                <div key={d.id} className="flex items-center justify-between border-b border-ink/10 py-2 text-sm">
                  <span>{d.name}</span>
          ...
          <Panel title={`Teams in ${current.name}`}>
            <div className="mb-4 flex flex-col gap-2">
              {seasonTeams.map((st) => (
                <div key={st.id} className="flex items-center justify-between gap-3 border-b border-ink/10 py-2 text-sm">
            ...
            <form action={updatePlayoffFormatForSeason!} className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              <Field label="Playoff teams">
                <input
                  ...
                  defaultValue={current.playoffTeamCount}
                  className={inputClass}
                />
              </Field>
              <Field label="Series lengths (comma-separated)">
                <input
                  name="playoffSeriesLengths"
                  defaultValue={current.playoffSeriesLengths.join(",")}
                  className={inputClass}
                />
              </Field>
