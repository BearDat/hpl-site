...
import { generateFirstRound, generateNextRound, createSeriesManually, } from "@/lib/actions/playoffs";
    ...
    return (<div>
      ...
      <Panel title="Generate Bracket">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {latestRound !== undefined && (<form action={generateNextRound} className="flex items-center gap-2">
              <input type="hidden" name="seasonId" value={season.id}/>
      ...
      {roundNumbers.map((round) => (<Panel key={round} title={rounds.get(round)[0]?.roundName ?? `Round ${round}`}>
          {rounds.get(round).map((s) => (<PlayoffSeriesCard key={s.id} series={s}/>))}
        </Panel>))}
